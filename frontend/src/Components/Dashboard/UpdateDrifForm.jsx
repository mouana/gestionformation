...
const UpdateDrifForm = () => {
  ...
  useEffect(() => {
    const fetchDrifData = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:8000/api/responsable-drif/${id}`, {
          headers: {
        ...
        console.error('Error fetching DRIF data:', err);
        setError('Erreur lors du chargement des données du responsable DRIF');
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Update the DRIF responsable
      await axios.put(`http://127.0.0.1:8000/api/responsable-drif/${id}`, {
      ...
      console.error('Error updating DRIF responsable:', err);
      setError(err.response?.data?.message || 'Une erreur est survenue lors de la mise à jour du responsable DRIF');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex min-h-screen bg-gray-100">
      <main className="flex-1 p-6">
        <div className="max-w-4xl mx-auto bg-white rounded-lg shadow p-6">
          <h1 className="text-2xl font-semibold mb-6">Modifier le Responsable DRIF</h1>
